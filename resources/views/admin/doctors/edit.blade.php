...
@section('content')
    <main class="app-content">
        <div class="app-title">
            <h1><i class="fa fa-edit"></i> {{ __('تعديل بيانات الطبيب') }}</h1>
        </div>
            ...
            <form method="POST" action="{{ route('admin.doctors.update', $doctor->doctor_id) }}">
                ...
                <div class="row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" name="first_name" class="form-control" value="{{ old('first_name', $doctor->user->first_name) }}">
                        @error('first_name') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                    <div class="form-group col-md-6">
                        <label>اسم العائلة</label>
                        <input type="text" name="last_name" class="form-control" value="{{ old('last_name', $doctor->user->last_name) }}">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <a href="{{ route('admin.doctors.index') }}" class="btn btn-secondary">{{ __('رجوع') }}</a>
                    </div>
                </div>
            </form>
        </div>
